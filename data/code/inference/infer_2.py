...
infer_dir = os.path.join(ROOT, 'user_data', 'dataV1', 'infer')
result_file_path = os.path.join(ROOT, 'raw_data', '测试集文件', 'result.tif')
tif2pngs = Tif2Pngs(result_file_path, os.path.join(infer_dir, 'images'), inference=True, stride=128)
tif2pngs.process_tif()

model_path = os.path.join(ROOT, 'user_data', 'models', 'model_3c_aug.pth')
model_config = {
    'model': Unet,
...
output_path = os.path.join(ROOT, 'user_data', 'submission_2.tif')
